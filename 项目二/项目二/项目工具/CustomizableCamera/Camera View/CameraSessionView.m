...
-(void)cameraSessionManagerFailedToCaptureImage {
    ...
    if ([self.delegate respondsToSelector:@selector(cameraSessionViewFailedToCaptureImage)]) {
        [self.delegate cameraSessionViewFailedToCaptureImage];
    }
}
...
-(void)cameraSessionManagerDidReportAvailability:(BOOL)deviceAvailability forCameraType:(CameraType)cameraType {
    ...
    if (!deviceAvailability) {
        if ([self.delegate respondsToSelector:@selector(cameraSessionViewNotFindCamera)]) {
            [self.delegate cameraSessionViewNotFindCamera];
        }
    }
}
